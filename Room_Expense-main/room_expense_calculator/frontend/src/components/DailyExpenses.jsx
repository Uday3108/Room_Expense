...
const CATEGORIES = ['Groceries', 'Vegetables', 'Milk', 'Gas', 'Snacks', 'Other']
...
export default function DailyExpenses({ month }) {
  ...
  const handleSubmit = async (data) => {
    if (editItem) {
      await updateDailyExpense(editItem.id, data)
    } else {
      await createDailyExpense(data)
    }
    setShowForm(false)
    setEditItem(null)
    load()
  ...
  return (
    <div className="card">
      ...
      {loading ? (
        ...
        <div className="empty-state"><p>No daily expenses found. Add one!</p></div>
      ) : (
